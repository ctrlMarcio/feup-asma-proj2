// from https://github.com/projectmesa/mesa/tree/main/examples/boid_flockers/boid_flockers
var ContinuousVisualization = function (width, height, context) {
  ...
  this.draw = function (objects) {
		...
		for (var i in objects) {
			var p = objects[i]
			if (p.Shape == 'rect') this.drawRectange(p.x, p.y, p.w, p.h, p.Color, p.Filled)
			if (p.Shape == 'circle') this.drawCircle(p.x, p.y, p.r, p.Color, p.Filled)
		}
	}
...
var Simple_Continuous_Module = function (canvas_width, canvas_height) {
  ...
  const canvasDraw = new ContinuousVisualization(
    canvas_width,
    canvas_height,
    context
  );
